...
# Obtendo as medidadas de dispersão da renda 
variancia_renda = np.var(array_financeira_renda)
distancia_var_renda = variancia_renda / (media_renda**2)  #dois ** é para elevar ao quadrado
desvio_padrao_renda = np.std(array_financeira_renda)
coeficiente_var_renda = desvio_padrao_renda / media_renda

# Obtendo as medidadas de dispersão do valor emprestado
variancia_Vlr_emprestado = np.var(array_financeira_Vlr_emprestado)
distancia_var_Vlr_emprestado = variancia_Vlr_emprestado / (media_Vlr_emprestado**2)  #dois ** é para elevar ao quadrado
...
print(f"A distancia da variância X média das rendas dos clientes é {distancia_var_renda:.2f}")
print(f"O desvio padrão das rendas dos clientes é {desvio_padrao_renda:.2f}")
print(f"O coeficiente da variação das rendas dos clientes é {coeficiente_var_renda:.2f}")
...
print(f"A distancia da variância X média dos empréstimos dos clientes é {distancia_var_Vlr_emprestado:.2f}")
print(f"O desvio padrão dos empréstimos dos clientes é {desvio_padrao_Vlr_emprestado:.2f}")
print(f"O coeficiente da variação dos empréstimos dos clientes é {coeficiente_var_Vlr_emprestado:.2f}")
...
plt.text(0.1,0.4,f'Distância entre Variância e Média  das Rendas{distancia_var_renda}',fontsize=12)
plt.text(0.1,0.3,f'Coeficiente de variação das Rendas{coeficiente_var_renda}',fontsize=12)


#Posição 04: Medidadas descritivas dos empréstimos
plt.subplot(2,2,4)
plt.text(0.1,0.9,f'Média dos Empréstimos {media_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.8,f'Mediana dos Empréstimo {mediana_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.7,f'Distância entre Média e Mediana dos Empréstimo {distancia_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.6,f'Maior valor dos Empréstimo {maior_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.5,f'Menor valor dos Empréstimo {menor_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.4,f'Distância entre Variância e Média dos Empréstimo {distancia_var_Vlr_emprestado}',fontsize=12)
plt.text(0.1,0.3,f'Coeficiente de variação dos Empréstimo {coeficiente_var_Vlr_emprestado}',fontsize=12)
